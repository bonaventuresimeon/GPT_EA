...
| ADR | Status | Supersedes | Superseded by | Decision |
|---|---|---|---|---|
...
- **[ADR_SUPERSESSION_POLICY.md](ADR_SUPERSESSION_POLICY.md)** — numbering, status, supersession, history-preservation and cycle rules.
- **[ADR_TEMPLATE.md](ADR_TEMPLATE.md)** — required structure for new decisions.
- `ADR_REGISTRY.json` — machine-readable ADR graph used by static governance checks.
...
Both the old and new ADR must carry reciprocal `Superseded by` / `Supersedes` metadata; old ADRs remain in the repository.
