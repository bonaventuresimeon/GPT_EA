...
Committed JSON configuration is consolidated into **`GPT_EA_DATA.json`**. Its `documents` object contains the 23 former registry/schema/template JSON documents keyed by their original filenames. Python tooling materializes ignored compatibility views automatically when legacy file paths are required.
