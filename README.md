...
By default, `InpSymbols="ALL"` discovers the connected broker/server's complete symbol catalog at runtime with MT5's own symbol registry rather than relying on a fixed broker list. Symbols outside Market Watch are admitted to discovery without trusting unavailable pre-selection metadata. Local MT5 custom symbols are excluded by default. Existing broker symbols may still be analyzed while temporarily disabled or close-only, but they remain **analysis-only** until live broker execution checks pass. Service-collateral instruments are excluded. `InpMaxBrokerUniverseSymbols=0` means the broker catalog is not artificially capped.

In full-universe mode the catalog refreshes automatically while the EA is running, so broker symbols added or removed after startup are picked up without restarting MT5. Common suffix/prefix conventions such as `.cash`, `m`, `c`, and other proprietary names are handled by exact discovery plus broker metadata/canonical normalization rather than by assuming a single broker naming scheme.

Classification combines broker symbol name, description/path, base/profit/margin currencies and `SYMBOL_TRADE_CALC_MODE`. The canonical classes are **FX, METAL, INDEX, ENERGY, COMMODITY, CRYPTO, STOCK, ETF, FUTURE, BOND/RATE and OTHER**. Broker-native stock/ETF/future/bond identity takes precedence over descriptive underlying aliases; crypto aliases require token/quote structure. Common aliases cover major global indices, precious metals, oil/gas, softs/agriculture/industrial metals, major and emerging-market FX, liquid crypto, exchange stocks, futures and bonds/rates. Proprietary broker tickers can still be classified from MT5 contract metadata, while unknown executable instruments remain generically analyzable rather than being dropped.
