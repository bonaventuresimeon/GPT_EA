...
By default, `InpSymbols="ALL"` discovers the broker's complete symbol catalog at runtime with MT5's own symbol registry rather than relying on a fixed broker list. Symbols outside Market Watch are admitted to discovery without trusting unavailable pre-selection metadata, then exact trade mode is rechecked immediately after MT5 selects each instrument for analysis. Disabled and service-collateral instruments are skipped, close-only instruments are excluded by default, and long-only/short-only markets remain discoverable with direction restrictions enforced before execution. `InpMaxBrokerUniverseSymbols=0` means the broker catalog is not artificially capped.
